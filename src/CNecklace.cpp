...
int CNecklace::identifySegment(SSegment *segment, STrackedObject *object, CRawImage* image)
{
    ...
    float x[2][idSamples];
    float y[2][idSamples];
    float tmpX[2][idSamples];
    float tmpY[2][idSamples];
    ...
    int ID[2];
    SNecklace result[2];
    if (tmp[0].x < tmp[0].m0 || tmp[0].y < tmp[0].m0) return -1;
    for(int i=0;i<2;i++){
        ...
        for (int a = 0;a<idSamples;a++){
            tmpX[i][a] = tmp[i].x+(tmp[i].m0*cos((float)a/idSamples*2*M_PI)*tmp[i].v0+tmp[i].m1*sin((float)a/idSamples*2*M_PI)*tmp[i].v1)*2.0;
            tmpY[i][a] = tmp[i].y+(tmp[i].m0*cos((float)a/idSamples*2*M_PI)*tmp[i].v1-tmp[i].m1*sin((float)a/idSamples*2*M_PI)*tmp[i].v0)*2.0;
            if(tmpX[i][a] > tmpXmax){
                tmpXmax = tmpX[i][a];
                tmpXidx = a;
            }
        ...
        // rearrange position arrays so sampling starts always on the right
        for(int a = 0; a < idSamples; a++){
            x[i][a] = tmpX[i][(tmpXidx + a) % idSamples];
            y[i][a] = tmpY[i][(tmpXidx + a) % idSamples];
        }
        ...
        realCode[i][length] = 0;
        SNecklace plainId = {.id = ID[i], .rotation = 0, .hamming = 1};
        result[i] = plainId;
        // result[i] = get(ID[i]);
    }
    ...
    // decition tree to find correct solution
    ...
    if(debugSegment) printf("segIdx %d numPoints %.3f id %d variance %.3f ID %d\n", segIdx, numPoints[segIdx], result[segIdx].id, variance[segIdx], ID[segIdx]);
